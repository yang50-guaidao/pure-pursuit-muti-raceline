...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate inner/outer track bounds from a map image. "
                    "Outputs <pkg_root>/csv/<map_name>/{inner,outer}_bound.{npy,csv} + track.{npy,csv}.")
    parser.add_argument("--map", required=True,
                        help="map name (without extension); expects matching files in <pkg_root>/maps/")
    parser.add_argument("--ext", default=".pgm", help="map image extension (default: .pgm)")
    parser.add_argument("--num-lanes", type=int, default=2)
    parser.add_argument("--clockwise", action="store_true",
                        help="set if the raceline goes clockwise (default: counter-clockwise)")
    parser.add_argument("--inner-safe-dist", type=float, default=0.30,
                        help="meters of safety margin from inner wall (default: 0.30)")
    parser.add_argument("--outer-safe-dist", type=float, default=0.30,
                        help="meters of safety margin from outer wall (default: 0.30)")
    parser.add_argument("--opp-safe-dist", type=float, default=0.20,
                        help="meters of margin used to derive opp-detector bounds (default: 0.20)")
    parser.add_argument("--maps-dir", default=None,
                        help="override maps dir (default: <pkg_root>/maps/)")
    parser.add_argument("--out-dir", default=None,
                        help="override output dir (default: <pkg_root>/csv/)")
    args = parser.parse_args()

    # Resolve paths relative to pkg root (one level up from scripts/)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    pkg_root = os.path.abspath(os.path.join(script_dir, ".."))
    maps_dir = args.maps_dir or os.path.join(pkg_root, "maps")
    out_root = args.out_dir or os.path.join(pkg_root, "csv")

    input_map = args.map
    input_map_ext = args.ext
    num_lanes = args.num_lanes
    clockwise = args.clockwise
    inner_safe_dist = args.inner_safe_dist
    outer_safe_dist = args.outer_safe_dist
    opp_safe_dist = args.opp_safe_dist

    # Read map params
    yaml_file = os.path.join(maps_dir, input_map + ".yaml")
    ...
    # Read image
    img_path = os.path.join(maps_dir, input_map + input_map_ext)
    ...
    csv_folder = os.path.join(out_root, input_map)
